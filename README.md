...
- Depois implementei a API externa, Brasil API, com uma funcionalidade simples, onde ela verifica as datas de tasks dentro do projeto para ver se não são feriados ou afins.

- Depois adicionei o swagger para conseguir testar os endpoints visualmente

- A partir desse ponto, percebi que havia deixado muitos requisitos obrigatórios para trás, que, novamente, se refizesse o projeto, já os implementaria desde o começo.

- Adicionei autorização por roles, logout e refresh, paginação e ordenação, filtros avançados, 
globalexceptionhandler (substituí as exceptions que tinha criado como classe) para centralizar exceptions, bean validation para validação mais robusta, estatisticas e relatorios de projetos, 
